...
interface MysteryBoxBuyButtonProps {
  boxId: string;
  priceEth: string;
  verifiedAddress: string;
  onOpened: (profile: PublicPlayerProfile, result: MysteryBoxResult) => void;
}
...
/** Near-identical purchase flow to BuyButton, but a mystery box grants a
 * random item (or consolation shards) rather than the id you paid for — so
 * the success path calls back with what was actually granted instead of
 * assuming it. */
export function MysteryBoxBuyButton({ boxId, priceEth, verifiedAddress, onOpened }: MysteryBoxBuyButtonProps) {
    ...
    async (txHash: string, attempt: number) => {
      try {
        const res = await fetch("/api/rps/shop/verify-purchase", {
          ...
          body: JSON.stringify({ itemId: boxId, txHash }),
        });
        ...
        setState("success");
        onOpened(data.profile, {
          grantedItemId: data.mysteryBox?.grantedItemId ?? null,
          grantedShards: data.mysteryBox?.grantedShards ?? 0,
        });
      } catch {
        if (attempt >= MAX_POLLS) {
          setState("error");
